...
mainfont: Arial
monofont: Times New Roman #DejaVu Sans Mono  # have to install the Dejavu font
fontsize: 11pt
linestretch: 1.2
linkcolor: blue
...
max-width: 100%  # size of the figure
format:
  html: 
    embed-resources: true # if I want to share it, set it as true
  pdf:
    pdf-engine: xelatex
    ...
    margin-top: 0.8in
    margin-bottom: 0.8in
    toc: true   #toc is table of contents
    keep-tex: true
    include-in-header:
      text: |
        \usepackage{lineno}
#    include-before-body:   # to remove linenumbers
      #text: |              
        #\linenumbers
bibliography: references.bib #add bibliography in this file
csl: science.csl #change according to journal, csl file can be found on zotero
# save csl file in the working directory

# .docx format can be rendered by saving it as quarto render Manuscript.docx

# can switch betwwen visual and source
---

```{r include=FALSE}
source("../analysis/scripts/statistics_for_paper.R") # cd using .., goes back to the Gaspar directory
...
The references are stored in manuscript/references.bib (need to be defined in the Yaml header). This file will automatically updated when you insert a new reference through the Visual editor \> Insert \> Citations.[@du2024] . **DOI can directly be searched from Pubmed.**
...
#| echo: false # code will not be displayed in the final document, otherwise it will be included in the final doc

knitr::include_graphics("figures/Figure1.png") 
# these triple marks allow to insert fig to the text file
```
...
tt(resources_table, theme = "grid", width = 1, placement = "H") # tt is tinytable
...
# hypothes.is embed allows annotation
-->
```
```{=html}
